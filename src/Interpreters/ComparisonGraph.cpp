...
bool isFunctionNode(const QueryTreeNodePtr & node)
{
    return node->as<FunctionNode>() != nullptr;
}

bool isFunctionNode(const ASTPtr & node)
{
    return node->as<ASTFunction>() != nullptr;
}
...
ComparisonGraph<Node>::ComparisonGraph(const NodeContainer & atomic_formulas, ContextPtr context)
{
    ...
    for (const auto & atom_raw : atomic_formulas)
    {
        ...
        const auto indices = [&]() -> std::optional<std::pair<std::optional<size_t>, std::optional<size_t>>>
        {
            if constexpr (with_ast)
            {
                const auto * func = atom->template as<ASTFunction>();
                if (!func || func->arguments->children.size() != 2)
                    return std::nullopt;

                return std::pair{get_index(func->arguments->children[0], g), get_index(func->arguments->children[1], g)};
            }
            else
            {
                const auto * function_node = atom->template as<FunctionNode>();
                if (!function_node)
                    return std::nullopt;
                ...
                return std::pair{get_index(arguments[0], g), get_index(arguments[1], g)};
            }
        }();
        ...
        if (indices)
        {
            auto [index_left, index_right] = *indices;
    ...
    for (const auto & atom_raw : atomic_formulas)
    {
        ...
        const auto indices = [&]() -> std::optional<std::pair<size_t, size_t>>
        {
            if constexpr (with_ast)
            {
                const auto * func = atom->template as<ASTFunction>();
                ...
                return std::pair{
                    graph.node_hash_to_component.at(func->arguments->children[0]->getTreeHash()),
                    graph.node_hash_to_component.at(func->arguments->children[1]->getTreeHash())};
            }
            else
            {
                const auto * function_node = atom->template as<FunctionNode>();
                ...
                return std::pair{
                    graph.node_hash_to_component.at(QueryTreeNodePtrWithHash{arguments[0]}),
                    graph.node_hash_to_component.at(QueryTreeNodePtrWithHash{arguments[1]})};
            }
        }();
...
void ComparisonGraph<Node>::EqualComponent::buildConstants()
{
    ...
    for (size_t i = 0; i < nodes.size(); ++i)
    {
        ...
        if constexpr (with_ast)
            is_literal = node->template as<ASTLiteral>() != nullptr;
        else
            is_literal = node->template as<ConstantNode>() != nullptr;
